# Prompts Otimizados para GitHub Copilot no Visual Studio Code

Este repositório contém uma série de prompts otimizados para maximizar a eficácia do GitHub Copilot no Visual Studio Code, cobrindo cenários de manutenção, desenvolvimento e resolução de problemas em aplicações. As informações estão divididas em arquivos menores e organizados por pastas para otimizar o consumo de contexto pelo Copilot.
...
- `best_practices/`: Contém informações sobre as melhores práticas para usar o Copilot.
- `capabilities_limitations/`: Detalhes sobre o que o Copilot pode e não pode fazer.
- `prompts_by_category/`: Prompts organizados por tipo de funcionalidade (desenvolvimento, manutenção, etc.).
- `use_cases/`: Exemplos práticos de aplicação dos prompts em cenários reais.
