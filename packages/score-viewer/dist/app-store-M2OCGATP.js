  ...
  persist(
    (set, get) => ({
      ...
      getAvailableWidth: () => {
        ...
        switch (breakpoint) {
          case "mobile":
            return viewport.width - 16;
          // 8px margins each side
          case "tablet":
            return viewport.width - 32;
          // 16px margins each side
          default:
            return Math.min(1200, viewport.width - 48);
        }
      },
      // Cleanup
      cleanup: () => {
...
var addRecentFile = (file) => useAppStore.getState().addRecentFile(file);
export {
  addRecentFile,
  useAppStore
};
