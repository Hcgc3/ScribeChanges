// src/components/ScoreViewer.jsx
import React7, { useRef as useRef4, useEffect as useEffect3, useState as useState2, useCallback as useCallback6 } from "react";
...
// ../../src/lib/stores/score-store.js
import { create } from "zustand";
import { persist } from "zustand/middleware";
var useScoreStore = create(
  persist(
    (set, get) => ({
      // Current score state
      file: null,
      ...
      viewSettings: {
        ...
        totalPages: 1,
        viewMode: "page",
        // 'page' (fixed A4) | 'continuous' (future editing)
      ...
      applyPageSettings: async (payload = {}) => {
        ...
        try {
          ...
          try {
            totalPages = ((_b = (_a = osmd.GraphicSheet) == null ? void 0 : _a.MusicPages) == null ? void 0 : _b.length) || totalPages;
          } catch {
          }
      ...
      setZoom: (zoom) => {
        ...
        const max = get().viewSettings.maxZoom ?? 4;
      ...
      loadScore: async (file) => {
        try {
          ...
          const { addRecentFile } = await import("./app-store-M2OCGATP.js");
          addRecentFile({
...
import { useCallback as useCallback2, useRef, useEffect } from "react";
var useZoom = () => {
  ...
  const applyPendingZoom = useCallback2(() => {
    ...
    const velocityMultiplier = Math.min(Math.max(velocity * 0.1, 0.5), 3);
    const dynamicStep = baseStep * velocityMultiplier;
    const zoomDirection = delta > 0 ? 1 : -1;
    const newZoom = Math.max(
      viewSettings.minZoom ?? 0.25,
      Math.min(viewSettings.maxZoom ?? 4, viewSettings.zoom + dynamicStep * zoomDirection)
    );
    ...
    zoomTimeoutRef.current = setTimeout(() => {
      if (zoomVelocityRef.current > 2) {
        ...
        const momentumZoom = Math.max(
          viewSettings.minZoom ?? 0.25,
          Math.min(viewSettings.maxZoom ?? 4, newZoom + momentumDelta)
        );
  ...
  const smoothZoom = useCallback2((delta, centerX = 0, centerY = 0) => {
  ...
  const zoomTo = useCallback2((targetZoom, duration = 300) => {
  ...
  const zoomIn = useCallback2(() => {
    const step = (viewSettings.zoomStep ?? 0.1) * 2;
    const targetZoom = Math.min(viewSettings.maxZoom ?? 4, (viewSettings.zoom ?? 1) + step);
    zoomTo(targetZoom);
  }, [viewSettings.zoom, viewSettings.zoomStep, viewSettings.maxZoom, zoomTo]);
  const zoomOut = useCallback2(() => {
    const step = (viewSettings.zoomStep ?? 0.1) * 2;
  ...
  const resetZoom = useCallback2(() => {
    zoomTo(1);
    setTimeout(() => setPanState({ x: 0, y: 0 }), 150);
  }, [zoomTo, setPanState]);
  const fitToWidth = useCallback2((containerWidth, contentWidth = 800) => {
    const targetZoom = Math.min(
      viewSettings.maxZoom ?? 4,
      Math.max(viewSettings.minZoom ?? 0.25, containerWidth * 0.9 / contentWidth)
    );
    zoomTo(targetZoom);
  }, [viewSettings.minZoom, viewSettings.maxZoom, zoomTo]);
  const fitToHeight = useCallback2((containerHeight, contentHeight = 1100) => {
    const targetZoom = Math.min(
      viewSettings.maxZoom ?? 4,
...
// src/hooks/usePan.js
import { useCallback as useCallback3, useRef as useRef2, useEffect as useEffect2 } from "react";
var usePan = () => {
  const { panState, setPanState, startPan, endPan, resetPan, editMode, viewSettings } = useScoreStore();
  const freezeMode = !!(viewSettings == null ? void 0 : viewSettings.freezeMode);
  const momentumRef = useRef2({ x: 0, y: 0 });
  const lastMoveTimeRef = useRef2(0);
  const velocityRef = useRef2({ x: 0, y: 0 });
  const animationFrameRef = useRef2(null);
  const frameRequestedRef = useRef2(false);
  const pendingPointRef = useRef2(null);
  const processPendingPan = useCallback3(() => {
    ...
    const now2 = Date.now();
    const timeDelta = now2 - lastMoveTimeRef.current;
    lastMoveTimeRef.current = now2;
  ...
  const enhancedUpdatePan = useCallback3((x, y) => {
  ...
  const enhancedEndPan = useCallback3(() => {
  ...
  const panTo = useCallback3((targetX, targetY, duration = 300) => {
  ...
  const centerContent = useCallback3(() => {
    panTo(0, 0);
  }, [panTo]);
...
import { useState, useCallback as useCallback5, useRef as useRef3 } from "react";

// src/hooks/useMusicalAnalysis.js
import { useCallback as useCallback4 } from "react";
var useMusicalAnalysis = () => {
  const analyzeScore = useCallback4((osmdInstance) => {
  ...
  const generateControlPoints = useCallback4((analysis, containerRect) => {
...
var useEditMode = () => {
  ...
  const osmdInstanceRef = useRef3(null);
  const toggleEditMode = useCallback5((osmdInstance, containerElement) => {
  ...
  const toggleDistances = useCallback5(() => {
    setEditMode({
      ...editMode,
      showDistances: !editMode.showDistances
    });
  }, [editMode, setEditMode]);
  const startDrag = useCallback5((pointId, startPosition, event) => {
  ...
  const updateDrag = useCallback5((event) => {
    ...
    if (point.constraints) {
      ...
      if (point.constraints.minX !== void 0) newX = Math.max(newX, point.constraints.minX);
      if (point.constraints.maxX !== void 0) newX = Math.min(newX, point.constraints.maxX);
      if (point.constraints.minY !== void 0) newY = Math.max(newY, point.constraints.minY);
      if (point.constraints.maxY !== void 0) newY = Math.min(newY, point.constraints.maxY);
    }
    ...
    const updatedPoints = editMode.controlPoints.map(
      (p) => p.id === dragState.draggedPoint ? { ...p, position: newPosition } : p
    );
    setEditMode({
      ...editMode,
  ...
  const generateDistanceLines = useCallback5((draggedPoint, allPoints) => {
    const lines = [];
  ...
  const calculateDistanceMeasurements = useCallback5((point, startPos, currentPos) => {
...
var EditControls = ({ osmdInstance, containerRef }) => {
  const {
    editMode,
    toggleEditMode,
    toggleDistances,
  ...
  if (!editMode.isActive) {
    return /* @__PURE__ */ React4.createElement("div", { className: "absolute top-5 left-5 z-30" }, /* @__PURE__ */ React4.createElement(
      Button,
      {
        variant: "default",
        size: "sm",
        onClick: () => toggleEditMode(osmdInstance, containerRef == null ? void 0 : containerRef.current),
        className: "bg-green-600 hover:bg-green-700"
      },
      /* @__PURE__ */ React4.createElement(Edit3, { className: "w-4 h-4 mr-2" }),
      "Modo Edi\xE7\xE3o"
    ));
  }
  return /* @__PURE__ */ React4.createElement("div", { className: "absolute top-5 left-5 z-30 flex flex-col gap-2" }, /* @__PURE__ */ React4.createElement(
    Button,
    {
      variant: "destructive",
      size: "sm",
    ...
    Button,
    {
      ...
      title: "Mostrar/Ocultar Linhas de Dist\xE2ncia",
      className: editMode.showDistances ? "bg-blue-100" : ""
    },
    /* @__PURE__ */ React4.createElement(Ruler, { className: "w-4 h-4" })
    ...
    /* @__PURE__ */ React4.createElement(EyeOff, { className: "w-4 h-4" })
  ), /* @__PURE__ */ React4.createElement(
    Button,
    {
      variant: "outline",
    ...
    /* @__PURE__ */ React4.createElement(Eye, { className: "w-4 h-4" })
  )), editMode.musicalAnalysis && /* @__PURE__ */ React4.createElement("div", { className: "bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg max-w-xs text-sm" }, /* @__PURE__ */ React4.createElement("div", { className: "font-medium mb-1" }, "An\xE1lise Musical"), /* @__PURE__ */ React4.createElement("div", { className: "text-xs text-gray-600" }, /* @__PURE__ */ React4.createElement("div", null, "Compasso: ", editMode.musicalAnalysis.timeSignature.numerator, "/", editMode.musicalAnalysis.timeSignature.denominator), /* @__PURE__ */ React4.createElement("div", null, "Compassos: ", editMode.musicalAnalysis.measures.length), /* @__PURE__ */ React4.createElement("div", null, "Pontos de controlo: ", editMode.controlPoints.length)), editMode.selectedElement && /* @__PURE__ */ React4.createElement("div", { className: "mt-2 pt-2 border-t border-gray-200" }, /* @__PURE__ */ React4.createElement("div", { className: "font-medium text-xs" }, "Elemento Selecionado:"), /* @__PURE__ */ React4.createElement("div", { className: "text-xs text-gray-600" }, editMode.selectedElement.replace(/^(barline|beat|staff|corner)-/, "").replace(/-/g, " ")))));
};
var EditControls_default = EditControls;

// src/components/MusicalControlPoint.jsx
import React5 from "react";
var MusicalControlPoint = ({
  point,
  onDragStart,
  onDrag,
  onDragEnd,
  ...
  const handleMouseDown = (e) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    onDragStart == null ? void 0 : onDragStart(point.id, point.position, e);
  };
  ...
  const getTooltipText = () => {
    ...
    switch (point.type) {
      ...
      case "beat":
        return `Tempo ${((_a = point.beatPosition) == null ? void 0 : _a.toFixed(1)) || point.beatIndex + 1} - ${point.noteType || "nota"}`;
      case "staff":
        return `Pauta ${point.staffIndex + 1}`;
      case "corner":
        return "Canto da p\xE1gina";
      default:
        return point.type;
    }
  };
  return /* @__PURE__ */ React5.createElement(
    ...
    point.type === "barline" && /* @__PURE__ */ React5.createElement("div", { style: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "2px", height: "80%", backgroundColor: "rgba(255,255,255,0.8)" } }),
    point.type === "beat" && point.noteType && /* @__PURE__ */ React5.createElement("div", { style: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "4px", height: "4px", backgroundColor: "rgba(255,255,255,0.9)", borderRadius: "50%" } })
  );
};
var MusicalControlPoint_default = MusicalControlPoint;

// src/components/DistanceVisualization.jsx
import React6 from "react";
var DistanceVisualization = ({
  distanceVisualization,
  dragState,
  editMode,
  containerRect
}) => {
  if (!distanceVisualization.show || !dragState.isDragging) return null;
  return /* @__PURE__ */ React6.createElement("div", { className: "absolute inset-0 pointer-events-none z-40" }, distanceVisualization.lines.map((line, index) => /* @__PURE__ */ React6.createElement(
    "div",
    {
      key: `line-${index}`,
      className: "absolute",
  ...
  )), distanceVisualization.measurements.map((measurement, index) => /* @__PURE__ */ React6.createElement(
    "div",
    {
    ...
    /* @__PURE__ */ React6.createElement("div", { className: "flex flex-col items-center" }, /* @__PURE__ */ React6.createElement("div", { className: "font-mono" }, measurement.value), measurement.type && /* @__PURE__ */ React6.createElement("div", { className: "text-xs opacity-75 capitalize" }, measurement.type.replace("-", " ")))
  )), dragState.isDragging && dragState.draggedPoint && /* @__PURE__ */ React6.createElement(React6.Fragment, null, /* @__PURE__ */ React6.createElement(
    "div",
    {
      className: "absolute h-px bg-blue-400 opacity-60",
      style: { left: 0, right: 0, top: `${dragState.currentPosition.y}px`, borderTop: "1px dashed #60a5fa" }
    }
  ), /* @__PURE__ */ React6.createElement(
    "div",
    {
      className: "absolute w-px bg-blue-400 opacity-60",
      style: { top: 0, bottom: 0, left: `${dragState.currentPosition.x}px`, borderLeft: "1px dashed #60a5fa" }
    }
  ), /* @__PURE__ */ React6.createElement(
    "div",
    {
      className: "absolute w-2 h-2 bg-green-500 rounded-full opacity-75",
      style: { left: `${dragState.startPosition.x - 4}px`, top: `${dragState.startPosition.y - 4}px` }
    }
  ), /* @__PURE__ */ React6.createElement(
    "div",
    {
      className: "absolute w-2 h-2 bg-red-500 rounded-full",
      style: { left: `${dragState.currentPosition.x - 4}px`, top: `${dragState.currentPosition.y - 4}px` }
    }
  ), /* @__PURE__ */ React6.createElement("svg", { className: "absolute inset-0 w-full h-full", style: { pointerEvents: "none" } }, /* @__PURE__ */ React6.createElement(
    "line",
    {
  ...
  ))), editMode.controlPoints.filter((point) => point.id !== dragState.draggedPoint).map((point) => {
    ...
    return /* @__PURE__ */ React6.createElement(
      "div",
      {
        ...
        style: { left: `${point.position.x - 2}px`, top: `${point.position.y - 2}px` }
      }
    );
  }));
};
var DistanceVisualization_default = DistanceVisualization;
...
var ScoreViewer = () => {
  ...
  const pageRef = useRef4(null);
  const osmdRef = useRef4(null);
  const fileInputRef = useRef4(null);
  const viewportRef = useRef4(null);
  const sheetContentRef = useRef4(null);
  const {
    file,
    loadingState,
    error,
  ...
  const hasFile = !!(file && (typeof File !== "undefined" && file instanceof File || typeof file === "object" && typeof file.content === "string" && file.content.length > 0));
  const lockView = hasFile ? (viewSettings == null ? void 0 : viewSettings.lockView) !== false : false;
  const viewMode = (viewSettings == null ? void 0 : viewSettings.viewMode) || "page";
  const clampZoom = (z) => Math.max(0.2, Math.min(z, 2.5));
  const isContinuous = viewMode === "continuous";
  const [scrollMetrics, setScrollMetrics] = useState2({
    scrollTop: 0,
    scrollHeight: 0,
    clientHeight: 0,
    effectiveScrollHeight: 0
  });
  const baseContentHeightRef = useRef4(0);
  const lastMeasuredPagesRef = useRef4(0);
  const zoomValueRef = useRef4(1);
  const BOTTOM_MARGIN_PX = 50;
  const getVisiblePagesScaledHeight = useCallback6(() => {
    ...
    svgs.forEach((svg) => {
      ...
      const top = Math.max(r2.top, vpRect.top);
      const bottom = Math.min(r2.bottom, vpRect.bottom);
      if (bottom > top) visible += bottom - top;
    });
    return visible;
  }, []);
  const updateScrollMetrics = useCallback6(() => {
    const vp = viewportRef.current;
    if (!vp) return;
    ...
    if (lockView && !isDraggingScrollRef.current) {
      try {
        ...
        const svgs = container == null ? void 0 : container.querySelectorAll("svg");
        const pageCount = (svgs == null ? void 0 : svgs.length) || 0;
        if (pageCount > 0 && container) {
          const currentZoom2 = zoomValueRef.current || 1;
          if (currentZoom2 <= 0 || !isFinite(currentZoom2)) {
            console.warn("Invalid zoom value for measurement:", currentZoom2);
            return;
          }
          ...
          if (scaledSpan <= 0) {
            ...
            if (firstSvg && lastSvg) {
              ...
              const BOTTOM_MARGIN_PX2 = 40;
              const PAGE_GAP_PX = 32;
              const totalGapPx = pageCount > 1 ? PAGE_GAP_PX * (pageCount - 1) : 0;
              const pageSpan = lastRect.bottom - firstRect.top;
              scaledSpan = pageSpan + TOP_MARGIN_PX + BOTTOM_MARGIN_PX2 + totalGapPx;
            }
          }
          const unscaledSpan = scaledSpan / currentZoom2;
          if (unscaledSpan > 0 && isFinite(unscaledSpan) && unscaledSpan < 1e5) {
            const currentBase = baseContentHeightRef.current || 0;
            const sizeDifference = Math.abs(unscaledSpan - currentBase);
            const threshold = Math.max(10, currentBase * 0.01);
        ...
        console.error("Error measuring content height:", error2);
      }
    }
    const unscaledContentHeight = baseContentHeightRef.current || 0;
    const viewportHeight = vp.clientHeight;
    const totalScaledHeight = unscaledContentHeight * currentZoom;
    const allPagesFit = totalScaledHeight <= viewportHeight + 0.5;
    let maxScrollTop = 0;
    if (!allPagesFit) {
    ...
    const newMetrics = {
      ...
      effectiveScrollHeight: totalScaledHeight
      // visual total pages height
    };
    ...
    setScrollMetrics((prev) => {
      if (Math.abs(prev.scrollTop - newMetrics.scrollTop) > 1 || Math.abs(prev.scrollHeight - newMetrics.scrollHeight) > 1 || Math.abs(prev.clientHeight - newMetrics.clientHeight) > 1 || Math.abs(prev.effectiveScrollHeight - newMetrics.effectiveScrollHeight) > 1) {
        return newMetrics;
      }
      return prev;
    });
  }, [lockView, getVisiblePagesScaledHeight]);
  const ensureContainerSize = useCallback6(() => {
  ...
  }, [file, osmdInstance, setOSMDInstance, setMetadata, updateViewSettings]);
  useEffect3(() => {
    const vp = viewportRef.current;
    if (!vp) return;
    if (lockView) {
  ...
  const [isDraggingScroll, setIsDraggingScroll] = useState2(false);
  const scrollDragRef = useRef4({ startY: 0, startScrollTop: 0, thumbHeight: 0, trackHeight: 0 });
  const scrollTrackRef = useRef4(null);
  const isDraggingScrollRef = useRef4(false);
  const scrollThumbSnapshotRef = useRef4(null);
  ...
  }, [renderOptions, setOSMDInstance, setError, file]);
  useEffect3(() => {
    if (!osmdInstance) return;
    let timeoutId;
    let isRendering = false;
    let lastRenderTime = 0;
    const minRenderInterval = 1e3;
    const onResize = () => {
      if (isRendering) return;
      const now2 = Date.now();
      if (now2 - lastRenderTime < minRenderInterval) {
        return;
      }
      ...
      timeoutId = setTimeout(async () => {
        ...
        try {
          ...
          console.warn("Resize render error:", error2);
        } finally {
          isRendering = false;
        }
      }, 500);
  ...
  }, [osmdInstance, updateScrollMetrics]);
  useEffect3(() => {
    if (hasFile && osmdInstance) {
      loadScoreFile(file);
    }
  ...
  useEffect3(() => {
    ...
    const handleWheel = (e) => {
      ...
      if (!lockView) {
        ...
        const dx = -e.deltaX / z;
        const dy = -e.deltaY / z;
        panTo(((panState == null ? void 0 : panState.x) || 0) + dx, ((panState == null ? void 0 : panState.y) || 0) + dy, 0);
      }
    };
  ...
  }, [smoothZoom, zoom, panState == null ? void 0 : panState.x, panState == null ? void 0 : panState.y, panTo, lockView, file, osmdInstance]);
  useEffect3(() => {
    if (!file || !osmdInstance) return;
    const viewport = viewportRef.current;
    if (!viewport) return;
  ...
  useEffect3(() => {
    if (!file || !osmdInstance || !panState.isDragging) return;
    ...
    const onDrop = (e) => {
      ...
      if (files && files.length > 0) {
        ...
        if (/(\.(xml|musicxml|mxl))$/i.test(droppedFile.name)) {
          ...
          setError({
            type: "file",
            message: "Formato de ficheiro n\xE3o suportado",
  ...
  useEffect3(() => {
    ...
    const onGChange = (e) => {
      e.preventDefault();
      const cx2 = e.clientX ?? window.innerWidth / 2;
      const cy = e.clientY ?? window.innerHeight / 2;
      smoothZoom((e.scale ?? 1) > 1 ? 1 : -1, cx2, cy);
    };
    const onGEnd = (e) => {
      e.preventDefault();
    };
  ...
  const loadScoreFile = async (scoreFile) => {
    var _a, _b;
    if (!osmdInstance) {
    ...
    try {
      ...
      setError(null);
      const isMXL = (_b = (_a = scoreFile == null ? void 0 : scoreFile.name) == null ? void 0 : _a.toLowerCase) == null ? void 0 : _b.call(_a).endsWith(".mxl");
      let content;
      if (scoreFile.content) {
        ...
        throw new Error("Formato de ficheiro n\xE3o suportado");
      }
      if (typeof content === "string") {
        try {
          osmdInstance.rawMusicXML = content;
        } catch {
        }
      }
      if (isMXL) {
      ...
      if (!hasValidDimensions) {
        throw new Error("Container n\xE3o tem dimens\xF5es v\xE1lidas para renderiza\xE7\xE3o");
      }
      await new Promise((r2) => requestAnimationFrame(r2));
      try {
        if (osmdInstance.zoom !== 1) osmdInstance.zoom = 1;
      } catch {
      }
      ...
      const getTotalPages = (osmd) => {
        ...
        try {
          return ((_b2 = (_a2 = osmd == null ? void 0 : osmd.GraphicSheet) == null ? void 0 : _a2.Pages) == null ? void 0 : _b2.length) || ((_d = (_c = osmd == null ? void 0 : osmd.graphic) == null ? void 0 : _c.pages) == null ? void 0 : _d.length) || ((_f = (_e = osmd == null ? void 0 : osmd.GraphicalMusicSheet) == null ? void 0 : _e.Pages) == null ? void 0 : _f.length) || 1;
        } catch {
          return 1;
        }
      };
  ...
  const readFileContent = (file2) => new Promise((resolve, reject) => {
    var _a, _b;
    const reader = new FileReader();
    const name = ((_b = (_a = file2.name) == null ? void 0 : _a.toLowerCase) == null ? void 0 : _b.call(_a)) || "";
    reader.onload = (e) => resolve(e.target.result);
    reader.onerror = () => reject(new Error("Erro ao ler o ficheiro"));
    if (name.endsWith(".mxl")) reader.readAsArrayBuffer(file2);
    else reader.readAsText(file2);
  });
  const extractScoreMetadata = (osmd) => {
    var _a, _b, _c, _d;
    try {
      const sheet = (osmd == null ? void 0 : osmd.sheet) || (osmd == null ? void 0 : osmd.Sheet) || {};
      const safe = (val, fallback = "") => {
        ...
        try {
          ...
          if (typeof val === "object") {
            ...
            const s = (_a2 = val.toString) == null ? void 0 : _a2.call(val);
      ...
      const title = safe(sheet.TitleString) || safe(sheet.title) || "Sem t\xEDtulo";
      const composer = safe(sheet.ComposerString) || safe(sheet.composer) || "Compositor desconhecido";
      const firstMeasure = (_a = sheet.sourceMeasures) == null ? void 0 : _a[0];
      const timeSigObj = (_b = firstMeasure == null ? void 0 : firstMeasure.timeSignatures) == null ? void 0 : _b[0];
      const timeSignature = safe((_c = timeSigObj == null ? void 0 : timeSigObj.toString) == null ? void 0 : _c.call(timeSigObj)) || "4/4";
      const keySigObj = (_d = sheet.keySignatures) == null ? void 0 : _d[0];
      const keySignature = safe(keySigObj == null ? void 0 : keySigObj.keySignature) || safe(keySigObj) || "C major";
      ...
      const instruments = Array.isArray(sheet.instruments) ? sheet.instruments.map((i) => safe(i == null ? void 0 : i.name) || safe(i == null ? void 0 : i.NameString) || safe(i == null ? void 0 : i.Name) || "Instrumento").filter(Boolean) : [];
      return {
        title,
        composer,
        keySignature,
  ...
  const fitPageToViewportWidth = useCallback6(() => {
    const viewport = viewportRef.current;
    ...
    if (baseWidth > 0) {
      const targetZoom = Math.min(available / baseWidth, 3);
      zoomTo(targetZoom);
      requestAnimationFrame(() => centerContent());
    }
  }, [zoomTo, centerContent, zoom]);
  const fitPageToViewportHeight = useCallback6(() => {
    const viewport = viewportRef.current;
    const page = pageRef.current;
    if (!viewport || !page) return;
    const available = Math.max(0, viewport.clientHeight - 64);
    const rect = page.getBoundingClientRect();
    const baseHeight = rect.height / (zoom || 1);
    if (baseHeight > 0) {
      const targetZoom = Math.min(available / baseHeight, 3);
      zoomTo(targetZoom);
      requestAnimationFrame(() => centerContent());
    }
  }, [zoomTo, centerContent, zoom]);
    ...
    const applyTransforms = () => {
      ...
      try {
        if (lockView) {
          ...
          sheetEl.style.justifyContent = "center";
          sheetEl.style.alignItems = "center";
          if (pageEl) {
            pageEl.style.transform = `scale(${clampZoom(zoom || 1)})`;
            pageEl.style.transformOrigin = "50% 0";
            pageEl.style.transition = panState.isDragging ? "none" : "transform 0.2s ease-out";
          }
        } else {
          sheetEl.style.display = "flex";
          sheetEl.style.justifyContent = "flex-start";
          sheetEl.style.alignItems = "flex-start";
          sheetEl.style.transform = `translate(${panState.x || 0}px, ${panState.y || 0}px) scale(${clampZoom(zoom || 1)})`;
          sheetEl.style.transformOrigin = "0 0";
          sheetEl.style.transition = panState.isDragging ? "none" : "transform 0.25s ease-out";
          if (pageEl) {
            pageEl.style.transform = "none";
          }
        }
      } catch (error2) {
        console.warn("Error applying transforms:", error2);
      }
    };
    frameId = requestAnimationFrame(applyTransforms);
    return () => cancelAnimationFrame(frameId);
  }, [panState.x, panState.y, panState.isDragging, zoom, lockView, loadingState, file, osmdInstance]);
  const commonZoomLevels = [0.5, 1, 1.5];
  const cacheThreshold = 0.02;
  const { setSVGCacheEntry, getSVGCacheEntry } = useScoreStore();
  const captureCurrentSVG = useCallback6((z) => {
    try {
      const container = containerRef.current;
      if (!container) return;
      ...
      setSVGCacheEntry(z, svgString, {
        width: clone.getAttribute("width"),
        height: clone.getAttribute("height")
      });
    } catch (e) {
      console.warn("Error capturing SVG:", e);
    }
  }, [setSVGCacheEntry]);
  useEffect3(() => {
    if (!osmdInstance || !file) return;
    let isCancelled = false;
    const preCache = async () => {
      for (const z of commonZoomLevels) {
        if (isCancelled) break;
        if (getSVGCacheEntry(z)) continue;
        captureCurrentSVG(z);
      }
    };
    const id = setTimeout(preCache, 300);
    return () => {
      isCancelled = true;
      clearTimeout(id);
    };
  }, [osmdInstance, file, captureCurrentSVG, getSVGCacheEntry]);
  useEffect3(() => {
    const match = commonZoomLevels.find((l) => Math.abs(l - zoom) < cacheThreshold);
    if (!match) return;
    const cached = getSVGCacheEntry(match);
    if (!cached) return;
    const container = containerRef.current;
    if (!container) return;
    ...
    if (!current) return;
    try {
      const currentStr = new XMLSerializer().serializeToString(current);
      if (Math.abs(currentStr.length - cached.svg.length) > 50) {
        container.innerHTML = cached.svg;
      }
    } catch (error2) {
      console.warn("Error swapping cached SVG:", error2);
    }
  }, [zoom, getSVGCacheEntry]);
  const onScrollbarThumbMouseDown = useCallback6((e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const totalScaled = (baseContentHeightRef.current || 0) * (zoomValueRef.current || 1) || 1;
    const visibleScaled = getVisiblePagesScaledHeight();
    const ratio = totalScaled > 0 ? visibleScaled / totalScaled : 0;
    const rawThumbPx = trackRect.height * ratio;
    const thumbPx = Math.max(30, Math.min(rawThumbPx, trackRect.height));
    const thumbTravelDistance = Math.max(0, trackRect.height - thumbPx);
    scrollDragRef.current = {
      startY: e.clientY,
      startScrollTop: vp.scrollTop,
      thumbHeight: thumbPx,
      trackHeight: thumbTravelDistance
    };
    scrollThumbSnapshotRef.current = {
      totalHeight: totalScaled,
      clientHeight: visibleScaled,
      thumbPx,
      trackPx: trackRect.height
    };
    setIsDraggingScroll(true);
    isDraggingScrollRef.current = true;
    const onMove = (ev) => {
      const { startY, startScrollTop, trackHeight } = scrollDragRef.current;
      ...
      let maxScrollTop = 0;
      if (!allPagesFit) {
        maxScrollTop = unscaledContentHeight - viewportHeight / currentZoom + BOTTOM_MARGIN_PX / currentZoom;
        if (maxScrollTop < 0) maxScrollTop = 0;
      }
      const scrollRange = maxScrollTop;
      const scrollDelta = deltaY / (trackHeight || 1) * (scrollRange || 0);
      let newScrollTop = Math.min(Math.max(0, startScrollTop + scrollDelta), maxScrollTop);
    ...
    const onUp = () => {
      ...
      scrollThumbSnapshotRef.current = null;
      updateScrollMetrics();
  ...
  }, [updateScrollMetrics, getVisiblePagesScaledHeight]);
  const onScrollbarTrackMouseDown = useCallback6((e) => {
    if (!scrollTrackRef.current || e.target !== scrollTrackRef.current) return;
    const trackEl = scrollTrackRef.current;
    const vp = viewportRef.current;
    if (!vp) return;
    ...
    const totalHeightForRatio = totalScaledHeight;
    const visibleScaled = Math.min(totalScaledHeight, viewportHeight);
    const ratio = totalHeightForRatio > 0 ? visibleScaled / totalHeightForRatio : 0;
    const thumbHeight = Math.max(30, rect.height * ratio);
    const travel = rect.height - thumbHeight;
    const targetScrollTop = (clickY - thumbHeight / 2) / (travel || 1) * maxScrollTop;
    vp.scrollTop = Math.min(Math.max(0, targetScrollTop), maxScrollTop);
    updateScrollMetrics();
  }, [updateScrollMetrics]);
  useEffect3(() => {
    if (!lockView) return;
    const id = requestAnimationFrame(() => updateScrollMetrics());
    return () => cancelAnimationFrame(id);
  }, [zoom, lockView, updateScrollMetrics]);
  useEffect3(() => {
    if (lockView) updateScrollMetrics();
  }, [zoom, lockView, updateScrollMetrics]);
  return /* @__PURE__ */ React7.createElement("div", { className: `music-sheet-container ${!hasFile ? "upload-mode" : ""}`, style: { background: hasFile ? "#f5f5f5" : "#1e1e1e", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" } }, /* @__PURE__ */ React7.createElement(
    "input",
    {
      ref: fileInputRef,
      type: "file",
  ...
  ), /* @__PURE__ */ React7.createElement("div", { className: `sheet-viewport-wrapper ${lockView ? "lock-view" : ""}`, style: { width: "100%", maxWidth: "100vw", minHeight: "100vh", alignItems: "center", justifyContent: "center", display: "flex", position: "relative" } }, /* @__PURE__ */ React7.createElement(
    "div",
    {
      ref: viewportRef,
      className: `sheet-viewport ${lockView ? "lock-view" : ""} ${isLoading ? "loading-state" : !hasFile ? "upload-state" : error ? "error-state" : ""}`,
      "data-has-score": hasFile && loadingState === "success" ? "true" : "false",
      onMouseDown: hasFile && !lockView ? handleMouseDown : void 0,
      onTouchStart: hasFile && !lockView ? handleTouchStart : void 0,
      onTouchMove: hasFile && !lockView ? handleTouchMove : void 0,
      onTouchEnd: hasFile && !lockView ? handleTouchEnd : void 0,
      style: { width: "100%", maxWidth: "100vw", minHeight: "100vh", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }
    },
    /* @__PURE__ */ React7.createElement("div", { ref: sheetContentRef, className: `sheet-content ${lockView ? "lock-view" : ""}`, style: { width: "100%", display: "flex", flexDirection: lockView ? "column" : "row", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "2rem 0" } }, /* @__PURE__ */ React7.createElement("div", { ref: pageRef, className: "a4-page", style: { width: "210mm", minHeight: "297mm", margin: "0 auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", boxSizing: "border-box", background: "transparent", position: "relative" } }, /* @__PURE__ */ React7.createElement("div", { ref: containerRef, className: `osmd-container ${!lockView ? "page-selection" : ""}`, style: { width: "100%", minWidth: "600px", minHeight: "400px", display: "flex", flexDirection: "column", alignItems: "center", boxSizing: "border-box", position: "relative", visibility: "visible", opacity: 1 } }))),
    isLoading && /* @__PURE__ */ React7.createElement("div", { className: "loading-overlay absolute inset-0 flex items-center justify-center z-50" }, /* @__PURE__ */ React7.createElement("div", { className: "flex flex-col items-center gap-4" }, /* @__PURE__ */ React7.createElement(Loader2, { className: "h-8 w-8 animate-spin" }), /* @__PURE__ */ React7.createElement("p", { className: "text-sm text-gray-600" }, loadingMessages[loadingState] || "A carregar..."))),
    error && /* @__PURE__ */ React7.createElement("div", { className: "error-overlay absolute inset-0 flex items-center justify-center z-50" }, /* @__PURE__ */ React7.createElement("div", { className: "flex flex-col items-center gap-4 max-w-md text-center" }, /* @__PURE__ */ React7.createElement(AlertCircle, { className: "h-8 w-8 text-red-500" }), /* @__PURE__ */ React7.createElement("div", null, /* @__PURE__ */ React7.createElement("h3", { className: "font-semibold text-red-600" }, error.message), error.details && /* @__PURE__ */ React7.createElement("p", { className: "text-sm text-gray-600 mt-2" }, error.details)), error.recoverable && /* @__PURE__ */ React7.createElement(Button, { onClick: () => setError(null), variant: "outline" }, "Tentar novamente"))),
    !hasFile && !isLoading && !error && /* @__PURE__ */ React7.createElement("div", { className: "upload-overlay absolute inset-0 flex items-center justify-center" }, /* @__PURE__ */ React7.createElement("div", { className: `flex flex-col items-center gap-4 p-8 rounded-lg border-2 border-dashed transition-colors ${dragOver ? "border-yellow-400 bg-yellow-50" : "border-white"}` }, /* @__PURE__ */ React7.createElement(Upload, { className: "h-12 w-12 text-gray-500" }), /* @__PURE__ */ React7.createElement("div", { className: "text-center" }, /* @__PURE__ */ React7.createElement("h3", { className: "font-semibold text-gray-800" }, "Carregar Partitura"), /* @__PURE__ */ React7.createElement("p", { className: "text-sm text-gray-600 mt-1" }, "Arraste um ficheiro .xml, .musicxml ou .mxl aqui")), /* @__PURE__ */ React7.createElement(Button, { onClick: handleFileUpload }, "Selecionar Ficheiro"))),
    editMode.isActive && editMode.controlPoints.map((point, index) => /* @__PURE__ */ React7.createElement(
      MusicalControlPoint_default,
      {
        key: index,
        point,
        index,
    ...
    distanceVisualization.isVisible && /* @__PURE__ */ React7.createElement(
      DistanceVisualization_default,
      {
        start: distanceVisualization.start,
        end: distanceVisualization.end,
        distance: distanceVisualization.distance
      }
    )
  ), lockView && scrollMetrics.scrollHeight > scrollMetrics.clientHeight && /* @__PURE__ */ React7.createElement(
    "div",
    {
      ref: scrollTrackRef,
    ...
    (() => {
      const trackEl = scrollTrackRef.current;
      const trackPx = trackEl ? trackEl.clientHeight : 0;
      const totalScaled = (baseContentHeightRef.current || 0) * (zoomValueRef.current || 1);
      const visibleScaled = getVisiblePagesScaledHeight();
      const ratio = totalScaled > 0 ? visibleScaled / totalScaled : 0;
      const rawThumbPxCalc = trackPx * ratio;
      const thumbPx = Math.max(30, Math.min(rawThumbPxCalc, trackPx));
      const actualMaxScroll = Math.max(0, scrollMetrics.scrollHeight - scrollMetrics.clientHeight);
      const travelPx = Math.max(0, trackPx - thumbPx);
      const progress = actualMaxScroll > 0 ? Math.min(1, scrollMetrics.scrollTop / actualMaxScroll) : 0;
      let topPx = progress * travelPx;
      if (topPx + thumbPx > trackPx) topPx = trackPx - thumbPx;
      if (topPx < 0) topPx = 0;
      return /* @__PURE__ */ React7.createElement(
        "div",
        {
          className: `custom-scroll-thumb ${isDraggingScroll ? "dragging" : ""}`,
          style: {
            height: `${thumbPx}px`,
  ...
  )), hasFile && loadingState === "success" && /* @__PURE__ */ React7.createElement("div", { className: "zoom-controls" }, /* @__PURE__ */ React7.createElement(
    Button,
    {
      variant: "outline",
      size: "icon",
    ...
    /* @__PURE__ */ React7.createElement(ZoomOut, { className: "h-4 w-4" })
  ), /* @__PURE__ */ React7.createElement("div", { className: "zoom-level" }, Math.round(zoom * 100), "%"), /* @__PURE__ */ React7.createElement(
    Button,
    {
      variant: "outline",
      size: "icon",
    ...
    /* @__PURE__ */ React7.createElement(ZoomIn, { className: "h-4 w-4" })
    ...
    /* @__PURE__ */ React7.createElement(RotateCcw, { className: "h-4 w-4" })
  ), /* @__PURE__ */ React7.createElement(
    Button,
    {
      variant: "outline",
      size: "icon",
      onClick: fitPageToViewportWidth,
      className: "zoom-button"
    },
    /* @__PURE__ */ React7.createElement(Maximize, { className: "h-4 w-4" })
  ), /* @__PURE__ */ React7.createElement(
    Button,
    {
      variant: "outline",
      size: "icon",
      onClick: fitPageToViewportHeight,
      className: "zoom-button"
    },
    /* @__PURE__ */ React7.createElement(Maximize2, { className: "h-4 w-4" })
  )), editMode.isActive && /* @__PURE__ */ React7.createElement(
    EditControls_default,
    {
      editMode,
      onToggleEditMode: () => {
      },
...
var ScoreViewer_default = ScoreViewer;

// ../../node_modules/.pnpm/tone@15.1.22/node_modules/tone/build/esm/version.js
var version = "15.1.22";

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-extended-exponential-ramp-to-value-automation-event.js
var createExtendedExponentialRampToValueAutomationEvent = (value, endTime, insertTime) => {
  return { endTime, insertTime, type: "exponentialRampToValue", value };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-extended-linear-ramp-to-value-automation-event.js
var createExtendedLinearRampToValueAutomationEvent = (value, endTime, insertTime) => {
  return { endTime, insertTime, type: "linearRampToValue", value };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-set-value-automation-event.js
var createSetValueAutomationEvent = (value, startTime) => {
  return { startTime, type: "setValue", value };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-set-value-curve-automation-event.js
var createSetValueCurveAutomationEvent = (values, startTime, duration) => {
  return { duration, startTime, type: "setValueCurve", values };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-target-value-at-time.js
var getTargetValueAtTime = (time, valueAtStartTime, { startTime, target, timeConstant }) => {
  return target + (valueAtStartTime - target) * Math.exp((startTime - time) / timeConstant);
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/exponential-ramp-to-value-automation-event.js
var isExponentialRampToValueAutomationEvent = (automationEvent) => {
  return automationEvent.type === "exponentialRampToValue";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/linear-ramp-to-value-automation-event.js
var isLinearRampToValueAutomationEvent = (automationEvent) => {
  return automationEvent.type === "linearRampToValue";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/any-ramp-to-value-automation-event.js
var isAnyRampToValueAutomationEvent = (automationEvent) => {
  return isExponentialRampToValueAutomationEvent(automationEvent) || isLinearRampToValueAutomationEvent(automationEvent);
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/set-value-automation-event.js
var isSetValueAutomationEvent = (automationEvent) => {
  return automationEvent.type === "setValue";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/set-value-curve-automation-event.js
var isSetValueCurveAutomationEvent = (automationEvent) => {
  return automationEvent.type === "setValueCurve";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-value-of-automation-event-at-index-at-time.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-end-time-and-value-of-previous-automation-event.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/cancel-and-hold-automation-event.js
var isCancelAndHoldAutomationEvent = (automationEvent) => {
  return automationEvent.type === "cancelAndHold";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/cancel-scheduled-values-automation-event.js
var isCancelScheduledValuesAutomationEvent = (automationEvent) => {
  return automationEvent.type === "cancelScheduledValues";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-event-time.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-exponential-ramp-value-at-time.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-linear-ramp-value-at-time.js
var getLinearRampValueAtTime = (time, startTime, valueAtStartTime, { endTime, value }) => {
  return valueAtStartTime + (time - startTime) / (endTime - startTime) * (value - valueAtStartTime);
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/interpolate-value.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/get-value-curve-value-at-time.js
var getValueCurveValueAtTime = (time, { duration, startTime, values }) => {
  const theoreticIndex = (time - startTime) / duration * (values.length - 1);
  return interpolateValue(values, theoreticIndex);
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/guards/set-target-automation-event.js
var isSetTargetAutomationEvent = (automationEvent) => {
  return automationEvent.type === "setTarget";
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/classes/automation-event-list.js
...
// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-cancel-and-hold-automation-event.js
var createCancelAndHoldAutomationEvent = (cancelTime) => {
  return { cancelTime, type: "cancelAndHold" };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-cancel-scheduled-values-automation-event.js
var createCancelScheduledValuesAutomationEvent = (cancelTime) => {
  return { cancelTime, type: "cancelScheduledValues" };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-exponential-ramp-to-value-automation-event.js
var createExponentialRampToValueAutomationEvent = (value, endTime) => {
  return { endTime, type: "exponentialRampToValue", value };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-linear-ramp-to-value-automation-event.js
var createLinearRampToValueAutomationEvent = (value, endTime) => {
  return { endTime, type: "linearRampToValue", value };
};

// ../../node_modules/.pnpm/automation-events@7.1.11/node_modules/automation-events/build/es2019/functions/create-set-target-automation-event.js
  ...
  get() {
    ...
    Object.keys(defaults).forEach((attribute) => {
      if (Reflect.has(this, attribute)) {
        ...
        } else if (isArray(member) || isNumber2(member) || isString(member) || isBoolean(member)) {
...
var OmniOscillator = class _OmniOscillator extends Source {
  ...
  set count(count) {
    if (this._getOscType(this._oscillator, "fat") && isNumber2(count)) {
  ...
  set spread(spread) {
    if (this._getOscType(this._oscillator, "fat") && isNumber2(spread)) {
...
var PolySynth = class _PolySynth extends Instrument {
  constructor() {
    ...
    assert(!isNumber2(options.voice), "DEPRECATED: The polyphony count is no longer the first argument.");
...
var Sampler = class _Sampler extends Instrument {
  constructor() {
    ...
    Object.keys(options.urls).forEach((note) => {
      ...
      assert(isNote(note) || isNumber2(noteNumber) && isFinite(noteNumber), `url key is neither a note or midi pitch: ${note}`);
      if (isNote(note)) {
        const mid = new FrequencyClass(this.context, note).toMidi();
        urlMap[mid] = options.urls[note];
      } else if (isNumber2(noteNumber) && isFinite(noteNumber)) {
  ...
  _rescheduleEvents(after = -1) {
    this._state.forEachFrom(after, (event) => {
      ...
      if (event.state === "started") {
        ...
        if (this._loop === true || isNumber2(this._loop) && this._loop > 1) {
          duration = Infinity;
          if (isNumber2(this._loop)) {
...
function createChannel(index) {
  const gain = new Gain(0.8).connect(masterGain);
  const poly = new PolySynth(Synth, {
    oscillator: { type: "sine" },
    envelope: { attack: 5e-3, decay: 0.1, sustain: 0.5, release: 0.5 }
  });
  poly.connect(gain);
...
var forceResumeAudio = async () => {
  ...
  try {
    ...
    if (ctx.state !== "running") await ctx.rawContext.resume();
  } catch {
  }
};
var initTone = async () => {
  if (ready) return { synths, transport: Transport };
  await forceResumeAudio();
  limiter = new Limiter(-3).toDestination();
  masterGain = new Gain(0.8).connect(limiter);
  ensureChannels();
  Transport.bpm.value = 120;
  Transport.loop = false;
  ready = true;
  return { synths, transport: Transport };
};
var isToneReady = () => ready;
var getSynth = (ch = 0) => synths[ch];
var getTransport = () => Transport;
var setMasterVolume = (v) => {
  if (masterGain) masterGain.gain.value = Math.max(0, Math.min(1, v));
};
var getMasterVolume = () => masterGain ? masterGain.gain.value : 0;
var setChannelVolume = (ch, v) => {
  if (channelGains[ch]) channelGains[ch].gain.value = Math.max(0, Math.min(1, v));
};
var getChannelVolume = (ch) => channelGains[ch] ? channelGains[ch].gain.value : 0;
var playTestChord = async () => {
  await initTone();
  const now2 = now();
  synths[0].triggerAttackRelease(["C4", "E4", "G4"], "2n", now2);
};
var applyTempoMap = (tempoEvents = [], measureStartWholes = []) => {
  ...
  tempoEvents.forEach((evt) => {
    try {
      ...
      Transport.bpm.setValueAtTime(evt.bpm, seconds);
    } catch {
    }
  });
};
...
var loadToneSequence = async ({ events, tempo, tempoEvents, measureStartWholes }) => {
  await initTone();
  Transport.stop();
  Transport.cancel();
  ...
  if (Array.isArray(tempoEvents) && tempoEvents.length) applyTempoMap(tempoEvents, measureStartWholes || []);
  else if (typeof tempo === "number") Transport.bpm.value = tempo;
  if (!Array.isArray(events) || !events.length) return null;
  const irregular = hasIrregularDurations(events);
  ensureChannels();
  try {
    if (!irregular) {
      currentPart = new Part((time, value) => {
...
var startTransportFromBeginning = async () => {
  await initTone();
  Transport.stop();
  Transport.seconds = 0;
  Transport.start();
...
var stopTransport = () => {
  if (isToneReady()) Transport.stop();
};

// src/lib/osmdMidiExtractor.js
var STEP_TO_SEMITONE = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11 };
var TOLERANCE = 1e-4;
var VERTICAL_ENTRY_KEYS = ["verticalSourceStaffEntries", "VerticalSourceStaffEntries", "verticalStaffEntries", "verticalSourceStaffEntryContainers", "VerticalSourceStaffEntryContainers"];
function getMidiNumber(pitch) {
  ...
  try {
    ...
    console.error("Erro na convers\xE3o de pitch para MIDI:", e);
    return null;
  }
}
...
function extractFromMeasures(osmdInstance, tempo, measureStartWholes) {
  ...
  const events = [];
  const skipTiedContinuation = /* @__PURE__ */ new WeakSet();
          ...
          notes.forEach((note) => {
            try {
              ...
              try {
                ...
                tsWhole = measureStartWholes[mi] || 0;
              }
              ...
              events.push({
                ...
                durationSeconds,
                midi,
                pitch: (pitch == null ? void 0 : pitch.step) || (pitch == null ? void 0 : pitch.Step),
                octave: (pitch == null ? void 0 : pitch.octave) ?? (pitch == null ? void 0 : pitch.Octave),
                step: (pitch == null ? void 0 : pitch.step) || (pitch == null ? void 0 : pitch.Step),
                alter: (pitch == null ? void 0 : pitch.alter) ?? (pitch == null ? void 0 : pitch.Alter) ?? 0,
                velocity,
                instrument: "Default",
                instrumentIndex: 0,
                channel: 0,
...
function inferMeasureDuration(measure) {
  if (measure == null ? void 0 : measure.Duration) {
    ...
    staffEntries.forEach((se) => {
      ...
      voiceEntries.forEach((ve) => {
        ...
        notes.forEach((note) => {
          ...
          const ts = (() => {
            try {
              if (typeof src.getAbsoluteTimestamp === "function") return fractionToWhole(src.getAbsoluteTimestamp());
            } catch {
            }
...
function deepDebugContextSafe(label, measures) {
  try {
    const first = measures == null ? void 0 : measures[0];
    console.warn(label, {
      measures: (measures == null ? void 0 : measures.length) || 0,
...
function extractMidiEvents(osmdInstance, options = {}) {
  ...
  if (useSimplePath && sourceMeasures.length) {
    try {
      const divisions = (() => {
        var _a, _b, _c, _d;
        try {
          return ((_b = (_a = sourceMeasures[0]) == null ? void 0 : _a.Attributes) == null ? void 0 : _b.divisions) || ((_d = (_c = sourceMeasures[0]) == null ? void 0 : _c.attributes) == null ? void 0 : _d.divisions) || sourceMeasures[0].Divisions || sourceMeasures[0].divisions;
        } catch {
          return null;
        }
      })() || 2;
      ...
      sourceMeasures.forEach((measure, mi) => {
        ...
        const voiceTime = /* @__PURE__ */ new Map();
        staffEntryBlocks.forEach((v) => {
          ...
          staffEntries.forEach((se) => {
            ...
            voiceEntries.forEach((ve) => {
              ...
              const vId = (ve == null ? void 0 : ve.voiceId) || (ve == null ? void 0 : ve.VoiceId) || 0;
              if (!voiceTime.has(vId)) voiceTime.set(vId, 0);
              notes.forEach((note) => {
                const src = note.sourceNote || note.SourceNote || note;
                if (src.isRest || note.isRest) return;
                const pitch = src.Pitch || src.pitch || note.Pitch || note.pitch;
                if (!pitch) return;
                const midi = getMidiNumber(pitch);
                if (midi == null) return;
                const rawDur = src.Duration || src.duration || note.Duration || note.duration || src.Length || src.length;
                const durDivs = typeof rawDur === "number" ? rawDur : (rawDur == null ? void 0 : rawDur.numerator) && (rawDur == null ? void 0 : rawDur.denominator) ? rawDur.numerator / rawDur.denominator * divisions : divisions;
  ...
  if (!events.length) {
    ...
    if (complex.length) events = complex;
    else deepDebugContextSafe("[Debug ComplexPath measures dump]", sourceMeasures);
  }
  if (!events.length) {
    try {
      ...
      console.warn("[MIDI Extract Debug] brute force failed", e);
    }
  }
  if (!events.length) {
  ...
  if (!events.length) {
    if (typeof rawXML === "string" && rawXML.length > 0) {
      try {
        ...
        if (rawEvents.length) events = rawEvents;
        else console.warn("[MIDI Extract RawXML Early] Nenhum evento extra\xEDdo do XML cru");
      } catch (e) {
        console.warn("[MIDI Extract RawXML Early] Falha ao analisar raw XML", e);
      }
    } else {
      console.warn("[MIDI Extract RawXML Early] rawXML indispon\xEDvel", typeof rawXML, rawXML && rawXML.length);
    }
  }
...
var findMeasureBinarySearch = (whole, measureInfos) => {
  let low = 0;
  let high = measureInfos.length - 1;
  let result = measureInfos[0];
  while (low <= high) {
...
function extractMidiData(osmdInstance, options = {}) {
  ...
  const sheet = (osmdInstance == null ? void 0 : osmdInstance.sheet) || (osmdInstance == null ? void 0 : osmdInstance.Sheet);
  const sourceMeasures = pickArray(sheet, ["sourceMeasures", "SourceMeasures"]);
  const measureInfos = [];
  let acc = 0;
  for (let i = 0; i < sourceMeasures.length; i++) {
    ...
    let num = 4, den = 4;
    try {
      const ts = (m == null ? void 0 : m.activeTimeSignature) || (m == null ? void 0 : m.TimeSignature) || (m == null ? void 0 : m.timeSignature);
      if (ts) {
        num = ts.Numerator ?? ts.numerator ?? num;
        den = ts.Denominator ?? ts.denominator ?? den;
      } else if (Array.isArray(m == null ? void 0 : m.timeSignatures) && m.timeSignatures[0]) {
        num = m.timeSignatures[0].numerator ?? num;
        den = m.timeSignatures[0].denominator ?? den;
      }
    } catch {
    }
  ...
  const toTimeString = (startWhole) => {
    ...
    return `${mi.index}:${beatIndex}:${sixteenths}`;
  };
...
function deepScanForNotes(osmdInstance, tempoEvents, measureStartWholes) {
  var _a;
  const root = (osmdInstance == null ? void 0 : osmdInstance.sheet) || (osmdInstance == null ? void 0 : osmdInstance.Sheet) || osmdInstance;
  if (!root) return [];
    ...
    for (const k of Object.keys(obj || {})) {
      if (keyHintRegex.test(k)) {
        ...
        if (val && typeof val === "object") {
          if (typeof val.RealValue === "number") return val;
  ...
  while (queue.length) {
    ...
    if (pitchHolder) {
      ...
      if (midi != null) {
        ...
        const tempo = ((_a = tempoEvents == null ? void 0 : tempoEvents[0]) == null ? void 0 : _a.bpm) || 120;
  ...
  if (results.length) {
    ...
    console.warn("[MIDI Extract Debug][DeepScan] Nenhuma nota encontrada no scan gen\xE9rico");
  }
  return results;
}
function parseRawMusicXMLForNotes(rawXml) {
  ...
  while (plMatch = partListRegex.exec(xml)) {
    ...
    const name = ((_b = (_a = /<part-name>([\s\S]*?)<\/part-name>/i.exec(block)) == null ? void 0 : _a[1]) == null ? void 0 : _b.trim()) || "Instrument";
  ...
  partSegments.forEach((seg, partIndex) => {
    ...
    while (noteMatch = noteRegex.exec(seg.content)) {
      ...
      if (/<grace\b/i.test(block)) continue;
      ...
      const step = ((_b2 = (_a2 = /<step>([A-G])<\/step>/i.exec(block)) == null ? void 0 : _a2[1]) == null ? void 0 : _b2.toUpperCase()) || "C";
      const alter = parseInt(((_c = /<alter>([-+]?\d+)<\/alter>/i.exec(block)) == null ? void 0 : _c[1]) || "0", 10);
      const octave = parseInt(((_d = /<octave>(\d+)<\/octave>/i.exec(block)) == null ? void 0 : _d[1]) || "4", 10);
      let durDivs = parseFloat(((_e = /<duration>(\d+)<\/duration>/i.exec(block)) == null ? void 0 : _e[1]) || "0");
      const type = (_f = /<type>([a-z0-9\-]+)<\/type>/i.exec(block)) == null ? void 0 : _f[1];
      ...
      if (tm) {
        actualNotes = parseInt(tm[1], 10) || null;
        normalNotes = parseInt(tm[2], 10) || null;
      }
      if (!durDivs) {
        if (type) {
          ...
          if (factor != null) {
            ...
            if (dots === 1) durDivs *= 1.5;
            else if (dots === 2) durDivs *= 1.75;
            if (actualNotes && normalNotes && actualNotes > 0) {
              durDivs *= normalNotes / actualNotes;
            }
      ...
      if (hasFermata) durDivs *= 1.5;
      const maxDivs = localCurrentDivs * 32;
      if (durDivs > maxDivs) durDivs = maxDivs;
      const voice = parseInt(((_g = /<voice>(\d+)<\/voice>/i.exec(block)) == null ? void 0 : _g[1]) || "1", 10);
    ...
    for (const rn of rawNotes) {
      ...
      if (rn.tieStart && !rn.tieStop) {
        if (!tieMap.has(key)) tieMap.set(key, rn);
        else tieMap.set(key, rn);
      } else if (!rn.tieStart && rn.tieStop) {
        const startNote = tieMap.get(key);
        if (startNote) {
          startNote.durDivs += rn.durDivs;
          rn.durDivs = 0;
          tieMap.delete(key);
        }
      } else if (rn.tieStart && rn.tieStop) {
    ...
    const processed = rawNotes.filter((n) => n.durDivs > 1e-4);
    const voiceTime = /* @__PURE__ */ new Map();
    const voiceLastStart = /* @__PURE__ */ new Map();
    processed.forEach((n) => {
      const v = n.voice;
      ...
      if (n.isChord && !n.isRest) {
        startDivs = voiceLastStart.get(v) || startDivs;
      }
  ...
  if (allEvents.length) {
    ...
    console.warn("[RawXML Parser] Nenhum evento ap\xF3s parsing de partes.");
  }
  return allEvents;
}
export {
  DistanceVisualization_default as DistanceVisualization,
  EditControls_default as EditControls,
  MusicalControlPoint_default as MusicalControlPoint,
  ScoreViewer_default as ScoreViewer,
  applyTempoMap,
  extractMidiData,
  extractMidiEvents,
  forceResumeAudio,
